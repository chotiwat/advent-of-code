...
func (c *cart) String() string {
	return fmt.Sprintf("%d %d %d", c.row, c.col, c.direction)
}
...
func main() {
	...
	occupied := make(map[intPair]bool)
	var m [][]rune
	var carts []*cart
	...
	for scanner.Scan() {
		...
		for col, cell := range row {
			...
			occupied[intPair{len(m), col}] = true
			carts = append(carts, &cart{row: len(m), col: col, direction: d})
			row[col] = replacement
		}
		m = append(m, row)
	}
	...
	for {
		for _, c := range carts {
			occupied[intPair{c.row, c.col}] = false
			// fmt.Println(c.row, c.col)
			c.move(m[c.row][c.col])
			if occupied[intPair{c.row, c.col}] {
				// crash
				fmt.Printf("%d,%d\n", c.col, c.row)
				return
			}
			occupied[intPair{c.row, c.col}] = true
		}
