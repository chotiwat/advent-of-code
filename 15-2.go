...
func (u *unit) attack(b battlefield, power int) bool {
	targets := u.pick(b)
	if len(targets) == 0 {
		return false
	}
	...
	if target.hp <= 0 {
		...
		if target.kind == kindElf {
			return true // an elf died
		}
	}
	return false
}
...
func main() {
	...
	// we could do binary search, but this only takes ~1.5s
	for power := 4; power <= 200; power++ {
		if ans, ok := run(b, power); ok {
			fmt.Println(ans)
			return
		}
	}
...
func run(bb battlefield, power int) (int, bool) {
	// copy b and units
	var b battlefield
	var units []*unit
	h := &unitHeap{}
	...
	for _, row := range bb {
		...
		for _, col := range row {
			var u *unit
			if col.occupant == wall {
				u = wall
			} else if col.occupant != nil {
				uu := *(col.occupant)
				u = &uu
				heap.Push(h, u)
				units = append(units, u)
			}
			newRow = append(newRow, cell{occupant: u})
		}
		b = append(b, newRow)
	}
	...
	for turn := 1; true; turn++ {
		...
		for h.Len() > 0 {
			...
			if len(targets) == 0 {
				...
				// fmt.Println(sumHP, turn, power)
				return sumHP * (turn - 1), true
			}
			u.move(b, targets)
			if u.attack(b, power) {
				return 0, false
